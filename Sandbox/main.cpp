...
private:
	...
	void initImGui()
	{
		...
		for (uint32_t i = 0; i < m_ViewportImageViews.size(); i++)
			m_Dset[i] = ImGui_ImplVulkan_AddTexture(textureSampler, m_ViewportImageViews[i], VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
	}
	...
	void cleanupSwapChain() {
		...
		{
			vkDestroyFramebuffer(device, m_ViewportFramebuffers[i], nullptr);
		}
		...
		{
			vkDestroyImageView(device, m_ViewportImageViews[i], nullptr);
		}
	...
	void cleanup() {
		...
		vkDestroyPipeline(device, m_ViewportPipeline, nullptr);
	...
	void createViewportImage()
	{
		...
		for (uint32_t i = 0; i < swapChainImages.size(); i++)
		{
			...
			// Create the image
			// VkImage dstImage;
			if (vkCreateImage(device, &imageCreateCI, nullptr, &m_ViewportImages[i]) != VK_SUCCESS)
			{
				throw std::runtime_error("failed to create image!");
			}
			//vkCreateImage(device, &imageCreateCI, nullptr, &m_ViewportImages[i]);
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex, const VkDescriptorSet& descriptorSet) {
		VkCommandBufferBeginInfo beginInfo{};
		beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
	...
	void drawFrame() {
		...
		recordCommandBuffer(commandBuffers[currentFrame], imageIndex, descriptorSets[currentFrame]);

		{
			...
			vkCmdBindIndexBuffer(m_ViewportCommandBuffers[currentFrame], indexBuffer, 0, VK_INDEX_TYPE_UINT16);
			vkCmdBindDescriptorSets(m_ViewportCommandBuffers[currentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSets[currentFrame], 0, nullptr);
